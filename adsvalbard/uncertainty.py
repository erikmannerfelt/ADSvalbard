...
def apply_function(chunk_dir: Path, func, pred_col: str, var_cols: list[str], redo: bool = False):
    ...
    out_path = temp_dir / f"{chunk_dir.parts[-2]}/chunk_{chunk_nr}_{pred_col}_baseline_err.tif"

    tcorr_err_path = out_path.with_stem(out_path.stem.replace("_baseline_err", "_tcorr_err"))
...
def main(redo: bool = False):
    ...
    # pred_col = "trend_2013-2024_slope"
        ...
        gdal.BuildVRT(
            str(Path("temp.svalbard/filt/svalbard/mosaics_3584") / f"{pred_col}_baseline_err.vrt"),
            out_paths,
        )
...
def make_fit_err_vrt():
    ...
    temporal_err_vrt_dir = uncertainty_dir / "fit_err_vrts/"
    ...
    for chunk_dir in chunks_dir.glob("chunk_*_*"):
        ...
        for se_path in chunk_dir.glob("*_se.tif"):
            ...
            if tcorr_err_path.is_file():
                vrt_path = temporal_err_vrt_dir / se_path.name.replace("_se.tif", "_fit_err.vrt")
    ...
    for product_name in error_chunks:
        ...
        vrt_path = mosaic_dir / f"{product_name}_fit_err.vrt"
