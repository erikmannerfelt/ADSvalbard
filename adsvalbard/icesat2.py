...
def download_icesat2(
    ...
    response = icepyx.Query(
        product="ATL06",
        spatial_extent=[
            region[k] for k in ["xmin_lat", "ymin_lat", "xmax_lat", "ymax_lat"]
        ],
...
def read_h5(filepath) -> xr.Dataset:
    ...
    transformer = pyproj.Transformer.from_crs(
        pyproj.CRS.from_epsg(4326), pyproj.CRS.from_epsg(32633)
    )
    for key in reader.groups.keys():
        ...
        dataset = (
            xr.open_dataset(
                xr.backends.NetCDF4DataStore(
                    ground_track.groups.get("land_ice_segments")
                )
            )
        ...
        eastings, northings = transformer.transform(
            dataset.latitude.values, dataset.longitude.values
        )
...
def merge_icesat():
    ...
    data = xr.concat(
        [
            ds
            for ds in process_map(read_h5, filepaths, desc="Loading h5 files")
            if len(ds) > 0
        ],
        dim="time",
    ).sortby("time")
    ...
    with TqdmCallback(desc=f"Saving {output_path.name}"):
        data.chunk(time=int(1e5)).to_netcdf(
            output_path,
            encoding={k: {"zlib": True, "complevel": 5} for k in data.data_vars},
            compute=False,
        ).compute()
...
def grid_icesat():
    ...
    data = data.where(data["atl06_quality_summary"] == 0.0)
    ...
    thresh = 50
    data = data.where(
        ((xr.apply_ufunc(np.abs, data["northing"] - test_point["y"])) < thresh)
        & ((xr.apply_ufunc(np.abs, data["easting"] - test_point["x"])) < thresh),
        drop=True,
    )
    ...
    # return
    data = np.vstack(
        (data["easting"].values, data["northing"].values, data["h_li"].values)
    ).T
...
def get_is2_data(
    region_label: str, bounds: rio.coords.BoundingBox, crs: rio.CRS | None = None
):
    if crs is None:
    ...
    for filename in ["snow_free_svalbard.csv", "snow_cover_svalbard.csv"]:
        ...
        data = gpd.GeoDataFrame(
            data, geometry=data["geometry"].apply(shapely.wkt.loads), crs=4326
        ).to_crs(crs)
...
def filter_is2_data(
    ...
    keep_columns: list[str] = ["easting", "northing", "h_te_best_fit"],
) -> pd.DataFrame:
    ...
    close_in_time = np.abs(
        is2_subset["date"] - pd.to_datetime(dem_data["datetime"]).to_datetime64()
    ) < pd.Timedelta(days=30 * 6)
    is2_subset = is2_subset.where(
        close_in_time | (is2_subset["on_snow"] == 0), drop=True
    )
