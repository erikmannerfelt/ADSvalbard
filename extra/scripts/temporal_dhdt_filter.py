...
def make_dhdt_ideal(per_year: xr.Dataset, bins: np.ndarray) -> np.ndarray:
    ...
    with warnings.catch_warnings():
        ...
        signal = per_year["dhdt"].where(per_year["glacier_mask_bool"]).median("year").load().stack(xy=["x", "y"]).groupby(per_year["digitized"].stack(xy=["x", "y"])).median().dropna("digitized")
...
def generate_mask(stack: xr.Dataset, label: str, outlines: gpd.GeoDataFrame):
    ...
    with rio.open(output_path, "w", driver="GTiff", width=shape[1], height=shape[0], count=1, crs=rio.CRS.from_epsg(32633), transform=transform, dtype="uint32", compress="deflate", tiled=True) as raster:
...
def filter_glacier(
    ...
    for year in years:
        yearly["neighboring_dhdt"].loc[{"year": year}] = subset["dhdt"].sel(year=slice(year - temporal_consistency_years, year + temporal_consistency_years + 1)).median("year")

        ds= make_dhdt_ideal_lazy(subset.isel(year=np.argwhere(subset["year"].values == year).ravel()), bins=bins)

        arr = da.from_delayed(ds, shape=(subset["y"].shape[0], subset["x"].shape[0]), dtype=subset["dhdt"].dtype)

        yearly["dhdt_ideal"].loc[{"year": year}] = arr
        
    subset["temporally_consistent"] = np.abs(subset["dhdt"] - yearly["neighboring_dhdt"].sel(year=subset["year"])) < temporal_consistency_threshold

    subset["dhdt_ideal_res_abs"] = np.abs(subset["dhdt"] - yearly["dhdt_ideal"].sel(year=subset["year"]))
    
    subset["noise_level"] = noise_floor_multiplier * subset["dhdt_ideal_res_abs"].groupby("year").median(["x", "y"])
    subset["okay_noise"] = subset["dhdt_ideal_res_abs"] < subset["noise_level"]

    return ~(subset["okay_noise"] | subset["temporally_consistent"]).where(subset["glacier_mask_bool"], other=True)
...
def filter_stack(stack_path: Path, outline_path: Path | str, crs_epsg: int = 32633):
    ...
    with xr.open_dataset(stack_path, chunks="auto") as stack:
        ...
        outlines.geometry = outlines.geometry.transform(lambda geom: shapely.wkb.loads(shapely.wkb.dumps(geom, output_dimension=2)))
        outlines.index += 1

        stack.attrs["mask_ids"] = json.dumps(outlines["rgi_id"].to_dict())

        stack["glacier_mask"] = ("y", "x"), generate_mask(stack=stack, label=label, outlines=outlines)

        # Initialize the valid mask as if all values are valid
        stack["invalid_mask"] = ("year", "y", "x"), da.zeros(
            dtype="bool",
        ...
        for i, outline in outlines.iterrows():
            ...
            selection = dict(x=slice(outline_bounds.left, outline_bounds.right), y=slice(outline_bounds.top, outline_bounds.bottom))

            result = filter_glacier(stack.sel(**selection), glacier_index=i)

            stack["invalid_mask"].loc[selection] = stack["invalid_mask"].loc[selection] | result

        stack = stack.swap_dims(year="time")

        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir) / "data.nc"
            task = stack.to_netcdf(temp_path, encoding={k: {"zlib": True, "complevel": 5} for k in stack.data_vars}, compute=False)
...
def main(label: str = "dronbreen"):
    ...
    with xr.open_dataset(filtered_path, chunks="auto") as stack:
        ...
        stack["dhdt"] = stack["dh"] / np.abs(stack["ref_dem_year"] - stack["time"].broadcast_like(stack["ad_elevation"])).clip(min=1)
